...
fn card(root: &Path, c: &Connection) -> rusqlite::Result<String> {
 ...
 let note:String=c.query_row("SELECT text FROM annotations WHERE snapshot=?1 ORDER BY id DESC LIMIT 1",params![now],|r|r.get(0)).unwrap_or_else(|_|"unknown".into());
...
fn main() -> Result<(),Box<dyn std::error::Error>> {
 ...
 match cmd.as_str(){
  ...
  "note"=>{let text=a.collect::<Vec<_>>().join(" "); if text.is_empty(){return Err("usage: relay note <text>".into())}; let s=snapshot(&root); c.execute("INSERT INTO annotations(snapshot,text) VALUES(?1,?2)",params![s,text])?; print!("{}",card(&root,&c)?);}
  ...
  _=>println!("relay init | observe | watch [seconds] | compact | note <text> | status | resume | check <command>")
 }; Ok(())
}
