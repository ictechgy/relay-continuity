...
| P2 | Workflow dependency graph | Contract job precedes archive; all packaging/publish paths transitively depend on it; release jobs have timeouts and concurrency. |
| P3 | Portable Linux build | Release matrix uses `x86_64-unknown-linux-musl`; final public-name binary has no ELF `NEEDED` entry or `GLIBC_*` symbol string. |
| P4 | Older runtime smoke | The exact Linux artifact runs help/version and disposable-repository smoke in digest-pinned Ubuntu 22.04 and Debian 12 x86_64 containers. |
...
| D1 | Doctor JSON contract | Valid schema version 1 JSON; only documented keys/enums; no absolute paths, raw errors, branch/path/note content, or sentinel secrets. |
...
| D3 | Doctor exit semantics | Healthy required state exits 0; broken/drifted/unsafe required state exits nonzero; absent optional components are explicit and do not become false success. |
| A1 | AI-card adversarial corpus | ANSI/CRLF/control/RTL/zero-width/very-long branch, path, and note sentinels do not occur in automatic context; fixed hashes/counts remain. |
